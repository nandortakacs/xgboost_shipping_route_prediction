...
from shipping_route_predictor.config import EnvConfig, ModelType, WeatherConfig  
...
# ======================================================================
# AISDataset — trajectory loading and feature extraction
# ======================================================================
        ...
        * Consecutive duplicate cells are removed; paths < 2 dropped.
        ...
        for fpath in json_files:
            ...
            for rec in records:
                ...
                vessel_info = self.vessel_registry.get(imo, {}) if imo else {}
                ...
                spec = TrajectorySpec(
                    ...
                    vessel_loa=vessel_info.get("loa", 0.0) or 0.0,
                    vessel_company=vessel_info.get("operator", "") or "",
                    imo=imo,
                )
    ...
    def vessel_registry(self) -> Dict[int, Dict[str, Any]]:
        ...
        if self._vessel_registry is None:
            ...
            if not path or not os.path.isfile(path):
                log.warning(
                    "Vessel registry not found at %s — LOA / company will be defaults",
                    path,
                )
