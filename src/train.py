    ...
    def after_iteration(self, model, epoch, evals_log):
        ...
        if (
            ...
            try:
                ...
                log.warning("Failed to save checkpoint at round %d: %s", rnd, exc)
...
class XGBoostTrainer:
    ...
    def save_model(self, path: Optional[Path] = None) -> Path:
        ...
        if str(path).endswith(".json"):
            ...
            try:
                ...
                self.log.warning("Binary save failed: %s", exc)
        return path
    ...
    def _save_results(
        ...
        try:
            ...
            self.log.warning("Feature importance save failed: %s", exc)
        ...
        if self._progress_cb is not None and self._progress_cb.history:
            ...
            try:
                ...
                self.log.warning("Failed to save iteration metrics CSV: %s", exc)
            try:
                self._save_training_curves(history)
            except Exception as exc:
                self.log.warning("Failed to save training curves: %s", exc)
