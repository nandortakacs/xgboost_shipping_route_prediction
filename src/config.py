...
# Vessel metadata
_VESSELS_JSON = os.path.join(_DATA_DIR, "vessel_data.json")

# Operator speed lookup (knots by LOA bin)
_OPERATOR_SPEED_JSON = os.path.join(_DATA_DIR, "operator_speed_kn_by_loa_bin.json")
...
# ═══════════════════════════════════════════════════════════════════════════
# Shared colour palettes (used by visualize, gui, SHAP plots)
# ═══════════════════════════════════════════════════════════════════════════
...
class WeatherConfig:
    ...
    #   "loa_operator_table" — look up knots from operator_speed_json by operator + LOA bin
    #   "fixed"              — use fixed_speed_kmh for all vessels
    speed_source: str = "loa_operator_table"
    operator_speed_json: str = _OPERATOR_SPEED_JSON
    fixed_speed_kmh: float = 26.92   # fallback / used when speed_source="fixed"
...
class EnvConfig:
    ...
    # Vessel metadata
    vessels_json: Optional[str] = _VESSELS_JSON
...
class TrajectorySpec:
    ...
    vessel_loa: float                             # length overall (metres)
...
class InferenceConfig:
    ...
    # Human-readable labels for each runner (used by GUI & logs)
...
class EvalConfig:
    ...
    limit: Optional[int] = 3   # max trajectories to evaluate
