...
class FullEnvGraph(EnvGraph):
    ...
    * **Feature construction** – ``build_features()`` returns the same 196-dim
    ...
    def reset(self, trajectory: TrajectorySpec) -> None:
        ...
        self._vessel_speed_kmh = self.weather_cfg.fixed_speed_kmh
    ...
    def build_input_features(self) -> np.ndarray:
        ...
        * **[118–195]** history (6 × 13 = 78)
        ...
        # --- base features (14) ---
        dgoal_e, dgoal_n = self.scaled_vector(cur, goal, wcfg.goal_vector_norm_km)
    ...
    def _set_node_input_features(
        ...
        for slot in range(n):
            ...
            out[off + 3] = ve
            out[off + 4] = vn
            de, dn = self.scaled_vector((ni, nj), goal_rc, wcfg.goal_vector_norm_km)
            out[off + 5] = de
            out[off + 6] = dn
