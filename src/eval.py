...
class Evaluator:
    ...
    def __init__(self, cfg: EvalConfig) -> None:
        ...
        if cfg.include_company_baseline:
            try:
                ...
                log.warning("CompanyBaseline init failed (sailing package?): %s", exc)
    ...
    def _evaluate_model_on_trajectory(
        ...
        *run_fn* must return ``(result_dict | None, pred_latlon | None)``.
        ...
        try:
            ...
            log.warning("%s/%s failed on %s: %s", model_name, rollout_type, spec.name, exc)
            result, pred_latlon = None, None
    ...
    def _save_histogram(self, out_root: Path) -> None:
        ...
        try:
            ...
            log.warning("Histogram PNG failed: %s", exc)
        ...
        try:
            ...
            log.warning("Histogram CSV failed: %s", exc)
